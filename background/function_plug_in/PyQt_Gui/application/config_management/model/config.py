...
from function_plug_in.PyQt_Gui.application.common.thumbtack.providers import SingletonMeta


class ConfigModel(metaclass=SingletonMeta):
    """
    ...
    @staticmethod
    def _load_version(version_path):
        ...
        if version_match:
            ...
            # 使用正则表达式提取对应版本的注释内容
            pattern = rf'# ver{version}[\s\S]*?(?=\n# ver|\Z)'
            version_content = re.search(pattern, version_file_content)
            if version_content:
                version_content = version_content.group(0)
            else:
                logger(f"未找到与版本 {version} 对应的注释内容。")
        else:
            logger("未找到版本信息。")
        return version, version_content
    ...
    def get_value(self, key, default=None, content=None, ):
        """获取配置项的值"""
        if content:
